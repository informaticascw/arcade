...
VISIBLE_ROWS = 3
SCROLL_SPEED = 20
FADE_HEIGHT = (TILE_HEIGHT + TILE_MARGIN) // 8  # sterke overgang
...
def launch_game(path, cwd):
    if sys.platform.startswith("win"):
        subprocess.Popen(f'cd "{cwd}" && python "{path}"', shell=True)
    else:
        subprocess.Popen(f'cd "{cwd}" && python3 "{path}"', shell=True, executable="/bin/bash")
...
def draw_menu():
    ...
    for index, game in enumerate(games):
        ...
        if y + TILE_HEIGHT < top_margin - FADE_HEIGHT or y > top_margin + max_menu_height + FADE_HEIGHT:
            continue
