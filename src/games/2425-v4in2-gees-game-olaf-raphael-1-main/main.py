...
while running:
      ...
      for ball in balls:
        if (ball['x'] + BALL_WIDTH > paddle_x and 
          ...
          ball['y'] < paddle_y + PADDLE_HEIGHT):
           ...
           # Kant waar de bal op gaat berekenen
           offset = (ball['x'] + BALL_WIDTH / 2) - (paddle_x + PADDLE_WIDTH / 2)
           ball['speed_x'] = offset * 0.1
           ball['speed_y'] = -math.sqrt(ball['speed']**2 - ball['speed_x']**2) # pytagoras want je wil de verticale zijde/ball['speed_y'] berekenen
           # Minimal speed
           if -2 < ball['speed_x'] < 2:
      ...
      for ball in balls:
        ...
        while i < len(bricks):
            ...
            if (ball['x'] + BALL_WIDTH > bx and
                ...
                if brick['hits'] >= 2:
                    bricks.pop(i)
                    # Kans om een ball toe te voegen
    ...
    if not game_status_msg == 'Victory Royale':
      # ball counter text
      if len(balls) == 1:
        game_status_msg3 = 'Je hebt ' + str(len(balls)) + ' bal'
      else:
         game_status_msg3 = 'Je hebt ' + str(len(balls)) + ' ballen'
      # start message       
      for ball in balls:
      ...
      if level_Counter == 1 and level_Status == True:
        ...
        for ball in balls:
          ball['speed'] *= 1.1
        level_Status = False
      elif level_Counter == 4 and level_Status == True:
        brick_Generation(8,5)
        reset_Level_State()
        for ball in balls:
          ball['speed'] *= 1.1
        level_Status = False
      elif level_Counter == 5 and level_Status == True:
        brick_Generation(8,7)
        reset_Level_State()
        for ball in balls:
          ball['speed'] *= 1.5
        level_Status = False
      ...
      if level_Counter == 1:
        ...
        game_status_msg2 = 'Level 4'
      elif level_Counter == 5:
        game_status_msg2 = 'Final level'
