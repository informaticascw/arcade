...
powerup_interval: float = POWERUP_INTERVAL
...
def main(screen: pygame.Surface, fps_clock: pygame.time.Clock, fps: int = FPS, level: int = 0) -> bool | None:
    ...
    levels: List[List[Dict[str, int]]] = json.load(open(ROOT + '/assets/levels.json'))
    ...
    balls: List[Ball] = [
        Ball((100,100), (BALL_WIDTH, BALL_HEIGHT), (BALL_SPEED * 1, BALL_SPEED * 1), ball_img, True) for _ in range(init_ball_count)
    ]

    next_powerup: float = 1
    powerups: List[Powerup] = [
        # new_powerup_at((random.randint(0, width - POWERUP_WIDTH), -POWERUP_HEIGHT), (POWERUP_WIDTH, POWERUP_HEIGHT), random.randint(0,10), POWERUP_SPEED)
    ]
    ...
    if not quit and result != None:
        if not cheats.tainted:
            ...
            elif len(name) > 0: score.add_game_entry(name, int(gscore))
